...
test(
  ...
  {
    annotation: [
      { type: 'product-area', description: '试卷模板 / 生成试卷' },
      {
        type: 'summary',
        description:
      ...
      {
        type: 'precondition',
        description: '已有一份包含三条语音的试卷模板，并配置三个可独立选择的语音提供商。'
      }
    ]
  },
  ...
  async ({}, testInfo) => {
    await test.step('从试卷模板编辑器进入不带一级导航的生成设置页', async () => {
      ...
      await expect(page.getByRole('heading', { level: 1, name: '生成流程文档模板' })).toBeVisible()
      await expect(page.getByRole('navigation', { name: '主导航' })).toHaveCount(0)
      await expect(page.getByRole('list', { name: '试卷生成阶段' })).toContainText('生成设置')
    })

    await test.step('填写试卷名称并为三种音色分别选择提供商、模型和音色', async () => {
      ...
      await testInfo.attach('生成设置', {
        body: await page.screenshot({ fullPage: true }),
        contentType: 'image/png'
      })
    })

    await test.step('生成进行中离开页面需要确认，继续生成会保留当前任务', async () => {
    ...
    await test.step('单条语音自动尝试四次仍失败时立即中断', async () => {
      ...
      await testInfo.attach('语音任务中断', {
        body: await page.screenshot({ fullPage: true }),
        contentType: 'image/png'
      })
    })

    await test.step('从失败语音继续生成，不重新合成已完成任务', async () => {
    ...
    await test.step('未保存结果时关闭会提示丢失风险', async () => {
    ...
    await test.step('加入试卷库和导出文件是独立操作，完成后页面仍保持打开', async () => {
      ...
      await expect(page.getByText('试卷文件已导出', { exact: true })).toBeHidden({ timeout: 6_000 })
      await testInfo.attach('生成结果', {
        body: await page.screenshot({ fullPage: true }),
        contentType: 'image/png'
      })
    })

    await test.step('主动关闭流程后可以在试卷库找到生成结果', async () => {
      const result = page.getByRole('region', { name: '八年级英语听说练习' })
      await result.getByRole('button', { name: '关闭', exact: true }).click()
      await expect(page.getByRole('heading', { level: 1, name: '生成流程文档模板' })).toBeVisible()
