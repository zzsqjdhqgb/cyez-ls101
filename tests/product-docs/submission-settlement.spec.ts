...
test(
  ...
  {
    annotation: [
      { type: 'product-area', description: '作答记录 / 评分结算' },
      {
        type: 'summary',
        description:
  ...
  async ({}, testInfo) => {
    await test.step('从作答记录独立入口导入两份作答包', async () => {
      ...
      await testInfo.attach('未结算作答列表', {
        body: await page.screenshot({ fullPage: true }),
        contentType: 'image/png'
      })
    })

    await test.step('勾选多条记录后按顺序进入一次评分会话', async () => {
    ...
    await test.step('评分会话结束后进入沉浸式结算页', async () => {
      ...
      await testInfo.attach('评分结算', {
        body: await page.screenshot({ fullPage: true }),
        contentType: 'image/png'
      })
    })

    await test.step('选择下次结算会保留评分并返回未结算列表', async () => {
    ...
    await test.step('再次进入结算并将两份作答原子结算为一个批次', async () => {
      ...
      await testInfo.attach('已结算批次', {
        body: await page.screenshot({ fullPage: true }),
        contentType: 'image/png'
      })
    })

    await test.step('重新评分会删除原结果并把原始作答移回未评分列表', async () => {
