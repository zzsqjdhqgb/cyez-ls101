...
- Interface 变量使用 Template 内配置的别名，选择后插入 `[@alias.varName]`；候选项以 `alias.varName` 展示，和无前缀的局部变量区分。
...
- 补全列表支持方向键切换候选、`Enter` 或 `Tab` 确认、`Escape` 关闭，也支持鼠标选择。
- 局部候选来自当前 Template 定义作用域中的函数调用输出、录音输出和选择题输出；Interface 候选仅来自已配置 requirement 的 `acceptedVars`。
- 候选变量先按字段类型过滤：Interface 文本对应 `string`，Interface 图片对应 `file`，函数输出沿用其声明类型，录音与选择题输出分别为 `audio` 与 `choice`。
- 输入器直接读写结构化 `TextExpression` / `ValueExpression`。修改变量前后的普通文本时保留已有变量引用，不把整个表达式压平成 literal。
- 第一阶段已接入 Page 卡片内 Timeline，以及右侧节点属性中的 TTS、题干、选项、倒计时和录音时长字段；后续新增的可编辑表达式字段复用同一输入器。
- 函数调用节点的入参和出参映射同时在节点卡片与右侧节点属性中完整可编辑。字符串入参使用可拼接变量输入器，数字和文件入参使用不可拼接变量输入器；出参编辑调用方作用域中的变量名，并继续触发下游引用自动重写。
- 函数调用节点按内嵌函数资源的签名排列参数并显示类型。资源暂时不可用时，编辑器回退展示调用节点已有的入参和出参绑定，并给出状态提示。
- 编辑器修改函数调用输出名、选择题输出名或录音输出名时，自动重写当前定义作用域中对旧局部变量名的引用；这是编辑辅助行为，不改变编译器的变量绑定规则。

### Page 内容画布与共享渲染层

- 页面显示能力不放入 `exam-player` 后再由编辑器反向依赖。独立的 `@ls101/page-renderer` 作为共享底层，未来由 Template 编辑器和 ExamPlayer 分别依赖。
- 共享渲染层只负责 1200×800 页面设计面、百分比坐标换算、缩放容器、内容块定位和文本、图片、ChoiceView 的视觉原语，不接收作者态或编译态的完整领域模型。
- 页面几何字段 `x / y / width / height` 统一表示相对 1200×800 设计面的百分比。图片位于底层，ChoiceView 位于中层，文本位于顶层；同类型内容块按文档数组顺序叠放。
- Template 编辑器直接把作者态 `ContentDocument` 适配到共享渲染原语；文本表达式中的变量在画布中显示为 token，图片变量和尚不能解析的 ChoiceView 使用作者态占位视图。
- ExamPlayer 后续把编译后的 `ExamPage` 适配到同一共享渲染原语。页面切换、时间线、TTS、倒计时、录音和作答状态仍属于 ExamPlayer，不进入共享页面渲染层。
- 中间编辑区使用“结构 / 页面”标签页。只有选中 Page 节点时页面标签可用；Page 节点卡片提供图标按钮直接进入对应页面画布。
- 页面画布支持添加文本、图片和 ChoiceView，选择、拖动、缩放、复制、删除内容块，以及缩放和适合窗口。拖动和缩放只在指针释放时提交一次文档 mutation，避免污染撤销历史。
- 选中内容块后，右侧显示独立、可折叠的“内容块”属性区域。当前可编辑 ID、位置和尺寸；文本可编辑变量表达式、字号、粗体和对齐；图片可编辑文件表达式；ChoiceView 可编辑 free、focus 和 range 三种视口配置。
- 图片块具有独立的固定宽度和高度。实际图片保持原始比例，在图片框内使用 `contain` 方式居中缩放，长边撑满、短边留空，不裁剪或拉伸。
- ChoiceView 的“聚焦题目”模式不向用户显示 `scope / callPath / questionId`。属性栏按 Collector 的最终分页结果提供“第几页 / 第几题”两个选择器，编辑器自动把选择结果映射为底层稳定的 `questionRef`。
- ChoiceView 中所有页码配置均使用下拉框，不允许手填内部页序号。free 可选择默认初始页；range 的初始页只列出起止范围内页面，修改起止页时自动维持有效闭区间。
- 函数可以只定义选择题，由外部 Collector 收集和分页。函数内若包含 ChoiceView，则其展开范围内必须同时产生唯一 Collector；首版不允许函数内 ChoiceView 反向依赖调用方的 Collector。完整题组函数应在内部封装题目、Collector 和显示页面，调用方只能整体调用，不能修改函数内部题目。
- 内容块的新增、更新和删除继续使用现有 Template 文档 mutation，因此与节点编辑共用撤销、重做、dirty 状态和保存流程，不增加后端或存储格式。
...
当前实现已经提供函数库仓储和内置 release 启动初始化；安装清单登记“基础组件库” v2，包含当前 DSL 的三个不可拆分原子组件：框架、页面和选择题；同时登记临时的“示例组件库” v3，其中只有“标题页组合”和“选择题组合”两个普通函数。标题页组合提供文本与数字入参，并导出组合标题与倒计时秒数；选择题组合提供说明、题干和选项文本入参，并导出选择结果与题干文本。只有 `builtin:basic` 被应用层硬编码为单节点预设库，点击时直接插入节点；包括示例库在内的其他函数库条目都生成函数调用节点。
