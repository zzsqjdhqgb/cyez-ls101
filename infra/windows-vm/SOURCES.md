...
以下列出这套 Packer/Vagrant 环境直接引入的全部下载项，以及后续运行应用会触发的既有下载入口。核对日期：2026-09-09。本次只读取了官方文本资料和发布元数据，没有下载二进制/ISO，也没有执行它们。
...
| Windows Server ISO | Server 2022 Evaluation，English x64，Desktop Experience | [Microsoft Evaluation Center](https://www.microsoft.com/en-us/evalcenter/evaluate-windows-server-2022)。手动选择 ISO；实际 CDN 下载链接可能随发布更新或过期，不硬编码不明镜像站。自行记录具体 ISO 文件名、发布日期及官方可取得的摘要。 |
| VMware Tools ISO | 与 Server 2022 x64 兼容的 `windows.iso`，由用户选择版本 | [Broadcom 官方支持门户](https://support.broadcom.com/)、[VMware 官方 Tools 发行目录](https://packages.vmware.com/tools/releases/)。也可复制官方 Workstation 安装目录内的 `windows.iso`。填写所选 ISO 的 SHA-256；guest 额外检查 ISO 内 `setup64.exe` 的 Authenticode 有效且签发给 VMware/Broadcom。安装器和驱动均由此 ISO 提供。 |
| Vagrant VMware Desktop Ruby 插件 | `vagrant-vmware-desktop` `3.0.5` | [RubyGems 页面](https://rubygems.org/gems/vagrant-vmware-desktop/versions/3.0.5)、[包](https://rubygems.org/downloads/vagrant-vmware-desktop-3.0.5.gem)、[官方仓库](https://github.com/hashicorp/vagrant-vmware-desktop)。由 README 的显式命令安装到项目 `VAGRANT_HOME`，不是 Packer 插件。当前发布元数据不列额外 runtime gem 依赖；Vagrant 自带的解析器/底层库仍随其全局安装提供。 |

已读取官方清单并填入 `config.example.psd1` 的值（仍请自行核对）：
