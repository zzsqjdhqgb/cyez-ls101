...
$config = Get-Content -LiteralPath $Config -Raw -Encoding UTF8 | ConvertFrom-Json
...
$managementPasswordFile = Join-Path $env:TEMP ('ls101-mgmt-' + [Guid]::NewGuid().ToString() + '.txt')

function Invoke-Driver([string[]]$Arguments, [switch]$AllowFailure) {
  $result = Invoke-Native -File $config.node -Arguments (@($config.driver) + $Arguments)
...
try {
  ...
  Invoke-Step 'install-teacher' {
    Assert-That (Test-Path -LiteralPath $config.installer) 'the teacher installer was uploaded'
    $process = Start-Process -FilePath $config.installer -ArgumentList '/S' -Wait -PassThru
    Write-Log "teacher installer exit code: $($process.ExitCode)"
    Assert-That ($process.ExitCode -eq 0) 'the silent install exited 0'
    $recordPath = Join-Path $manifest 'installation.json'
    Assert-That (Test-Path -LiteralPath $recordPath) 'installation.json was published'
    $record = Get-Content -LiteralPath $recordPath -Raw | ConvertFrom-Json
    Assert-That ($record.release -like "$($config.releaseVersion)-*") "the installed release matches $($config.releaseVersion)"
  ...
  Invoke-Step 'service-registration' {
    $qc = Invoke-Native -File 'sc.exe' -Arguments @('qc', $config.serviceName)
    Write-Log "sc.exe qc $($config.serviceName)`n$($qc.Text)"
    Assert-That ($qc.Code -eq 0) 'sc.exe qc succeeded'
    $sidType = Invoke-Native -File 'sc.exe' -Arguments @('qsidtype', $config.serviceName)
    Write-Log "sc.exe qsidtype $($config.serviceName)`n$($sidType.Text)"
    ...
    $service = Get-CimInstance Win32_Service -Filter "Name='$($config.serviceName)'"
    ...
    Assert-That ($service.StartName -ieq $config.serviceAccount) "the service runs as $($config.serviceAccount)"
  ...
  Invoke-Step 'data-acl' {
    Assert-That (Test-Path -LiteralPath $dataDir) 'the installer created the data directory'
    $acl = Get-Acl -LiteralPath $dataDir
    Write-Log "data directory ACL:`n$($acl | Format-List | Out-String)"
    Assert-That ($acl.AreAccessRulesProtected) 'inheritance is disabled on the data directory'
    $identities = @($acl.Access | ForEach-Object { $_.IdentityReference.Value })
    Write-Log "data directory identities: $($identities -join '; ')"
    Assert-That (-not ($identities | Where-Object { $_ -match 'BUILTIN\\Users$|Authenticated Users|Everyone' })) 'no standard-user group holds a data directory rule'
    ...
    Assert-That ($identities | Where-Object { $_ -match 'LS101Lab' }) 'the service SID holds a data directory rule'
  ...
  Invoke-Step 'service-start' {
    ...
    Assert-That ($candidates.Count -ge 1) 'the service hosts the bundled Node process'
    $hostProcess = $candidates | Select-Object -First 1
  ...
  Invoke-Step 'clock-and-license-window' {
    ...
    $hostNow = [DateTime]::Parse($config.hostTime).ToUniversalTime()
  ...
  Invoke-Step 'initialize-service' {
    Assert-That (Test-Path -LiteralPath $config.invitationFile) 'the invitation code was uploaded'
    ...
    $inputFile = Join-Path $env:TEMP ('ls101-initialize-' + [Guid]::NewGuid().ToString() + '.json')
    $driverResult = Join-Path $env:TEMP ('ls101-initialize-' + [Guid]::NewGuid().ToString() + '-result.json')
    @{
      name    = 'LS101 Lab'
    ...
    try {
      ...
      $result = Invoke-Driver -Arguments @(
        ...
        '--activation-file', $config.invitationFile,
    ...
    Remove-Item -LiteralPath $config.invitationFile -Force -ErrorAction SilentlyContinue
    Assert-That (-not (Test-Path -LiteralPath $config.invitationFile)) 'the invitation code was removed from the guest'
  }

  # --- S8b/S12: the listener is real, on 0.0.0.0, and its key is independently verified ---------
  Invoke-Step 'listener-and-identity' {
    $listeners = @(Get-NetTCPConnection -State Listen -LocalPort $config.port -ErrorAction SilentlyContinue)
    Assert-That ($listeners.Count -ge 1) "the service listens on port $($config.port)"
...
'@ | Set-Content -LiteralPath $probePath -Encoding UTF8
  ...
  Invoke-Step 'restart-survives' {
    ...
    $restarted = Invoke-Native -File $runtimeNode -Arguments @($server, 'status', '--data-dir', $dataDir)
    Assert-That ($restarted.Code -eq 0) 'the control channel answered after the restart'
    $state = ConvertFrom-JsonOutput $restarted.Text
    Assert-That ($state.state -eq 'running') 'the restarted service reports running'
  ...
  Invoke-Step 'firewall-closed' {
    ...
    $matching = @(
      $allInbound |
        ...
        Where-Object { @($_.LocalPort) -contains $config.port -or @($_.LocalPort) -contains "$($config.port)" }
    )
    Write-Log "inbound rules covering port $($config.port): $($matching.Count)"
