...
- **M4（U1–U5，已实机全绿）**：同版本覆盖安装（无需升级准备）、版本变化的准入拒绝（准备缺失与目标版本不符两种形态）、经真实备份准备后的直接安装器升级、NSIS 卸载客户端保留服务与数据（需 `/S _?="<安装目录>"` 并轮询文件消失）、运行中卸载被拒、服务卸载后重装的 serverId/回执保留、第二遍机密扫描。学生端安装包在本轮只做"上传成功且非空"的校验，**没有安装**。

M1 尚未实现的是 S15（停止语义与在线设备）、S16（重启后自启动）、S17（端口占用）；M3（已安装产品的 CDP GUI）未实现。因此 guest 步骤总数是 **34（M1 15 + M2 11 + M4 8）**，而不是旧口径的 26。M4 已于 2026-09-21 在真实 Windows 宿主机上全绿（runId `1789991140621-bca543c3…`），并确认了两条部署步骤：升级后服务处于停止状态需显式启动，且仍处于维护模式需显式退出。
