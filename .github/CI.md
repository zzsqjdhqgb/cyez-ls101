...
The verification has four job ids: `test_suites` (a Windows matrix with two entries),
`canonical_docs`, `linux_electron_smoke`, and a final `Required quality gate`. The `test_suites`
...
the pinned Linux renderer container, and `linux_electron_smoke` runs the Linux packaged-Electron
...
canonical runs upload the regenerated product documentation as `canonical-product-docs-*` for the
