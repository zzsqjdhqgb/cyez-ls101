...
const VALIDATION_MESSAGES: Record<TemplateValidationErrorCode, string> = {
  ...
  DUPLICATE_INTERFACE_MANIFEST: '存在重复的题型定义',
  DUPLICATE_SCHEMA_DEFINITION: '存在重复的评分单元定义',
  DUPLICATE_FUNCTION_DEF: '存在重复的函数定义',
  INVALID_FUNCTION_RESOURCE_ID: '函数资源编号无效',
  FUNCTION_RESOURCE_ID_MISMATCH: '函数资源内容与编号不匹配',
  INVALID_INTERFACE_ALIAS: '题型别名格式无效',
  DUPLICATE_INTERFACE_ALIAS: '题型别名重复',
  UNKNOWN_INTERFACE: '引用的题型不存在',
  EMPTY_ACCEPTED_VARS: '题型至少需要接受一个变量',
  DUPLICATE_ACCEPTED_VAR: '题型接受了重复变量',
  UNKNOWN_INTERFACE_VAR: '题型变量不存在',
  INTERFACE_VAR_NOT_ACCEPTED: '题型变量未被当前模板接受',
  EMPTY_NODE_ID: '节点编号不能为空',
  DUPLICATE_NODE_ID: '节点编号重复',
  EMPTY_CONTENT_BLOCK_ID: '内容块编号不能为空',
  DUPLICATE_CONTENT_BLOCK_ID: '内容块编号重复',
  ...
  UNKNOWN_INTERFACE_ALIAS: '引用的题型别名不存在',
  INTERFACE_VARIABLE_IN_FUNCTION: '本地函数不能直接引用题型变量',
  ...
  EMPTY_CHOICE_OPTION_ID: '选择题选项编号不能为空',
  DUPLICATE_CHOICE_OPTION_ID: '选择题选项编号重复',
  INVALID_SCHEMA_USE_ID: '评分单元编号格式无效',
  DUPLICATE_SCHEMA_USE_ID: '评分单元编号重复',
  UNKNOWN_SCHEMA: '引用的评分单元不存在',
  MISSING_SCHEMA_INPUT_BINDING: '评分单元缺少题目输入绑定',
  UNKNOWN_SCHEMA_INPUT_BINDING: '评分单元包含未知题目输入绑定',
  MISSING_SCHEMA_ANSWER_BINDING: '评分单元缺少答案绑定',
  UNKNOWN_SCHEMA_ANSWER_BINDING: '评分单元包含未知答案绑定',
  SCHEMA_ANSWER_TYPE_MISMATCH: '评分单元答案绑定类型不匹配',
  INVALID_SCHEMA_ATTACHMENT_NAME: '评分单元附件变量名格式无效',
  DUPLICATE_SCHEMA_ATTACHMENT_NAME: '评分单元附件变量名重复',
  UNKNOWN_SCHEMA_ATTACHMENT: '引用的评分单元附件不存在',
  EMPTY_CHOICE_COLLECTOR: '选择题采集器没有收集到题目',
  EMPTY_CHOICE_COLLECTOR_PAGES: '选择题采集器没有分页',
  INVALID_CHOICE_PAGE_SIZE: '选择题分页大小无效',
  CHOICE_PAGE_TOTAL_MISMATCH: '选择题分页总数与题目数不一致',
  NESTED_CHOICE_COLLECTOR: '选择题采集器不能嵌套',
  MULTIPLE_CHOICE_COLLECTORS: '模板只能生成一份选择题元数据',
  UNCOLLECTED_CHOICE_QUESTIONS: '存在未被收集的选择题',
  CHOICE_VIEW_WITHOUT_META: '选择题视图缺少选择题元数据',
  FUNCTION_CHOICE_VIEW_WITHOUT_LOCAL_COLLECTOR: '函数内选择题视图缺少本地采集器',
...
const COMPILE_MESSAGES: Record<string, string> = {
  DUPLICATE_INTERFACE_BINDING: '同一个题型被绑定了多次',
  MISSING_INTERFACE_BINDING: '缺少题组',
  UNKNOWN_INTERFACE_BINDING: '选择了模板未使用的题组',
  INTERFACE_BINDING_ID_MISMATCH: '题组与模板要求不匹配',
  INTERFACE_INSTANCE_NOT_FOUND: '找不到所选题组',
  MISSING_INTERFACE_VALUE: '题组存在未填写的变量',
