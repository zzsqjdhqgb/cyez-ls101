...
export class LicenseService {
  ...
  getStatusSync(): LicenseStatus {
    if (this.currentTime() > this.expirationTime)
      return { state: 'expired', expiresAt: this.expiresAt }
    let value: unknown
    try {
      value = JSON.parse(readFileSync(this.options.storagePath, 'utf8'))
    } catch (error) {
    ...
    return isLicenseReceipt(value) && this.isValidReceipt(value)
      ? { state: 'active', expiresAt: this.expiresAt, activatedAt: value.activatedAt }
      : { state: 'not-activated', expiresAt: this.expiresAt }
  }
