...
本目录记录自动化测试的分层、运行约束和技术测试清单。产品行为及其截图证据位于[产品文档](../../product/README.md)，不在这里重复说明。
...
[Playwright 技术测试清单](./inventory.md)由 Playwright 的测试发现结果生成，只记录当前测试标题、数量和源码入口。具体步骤与断言以测试源码为准，不再维护平行的手工路径说明。

生成命令：

```bash
yarn docs:playwright-inventory
```
