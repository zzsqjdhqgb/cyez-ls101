...
`textGenerator` 可选。未配置时，浏览、草稿、手动实例编辑、JSON 覆盖和导入导出仍可使用；调用 AI 生成会抛出配置缺失错误。`imageGenerator` 同样可选；未配置时，AI 生成在需要图片时抛出配置缺失错误，`listImageGenerationProviders()` 返回空列表，`generateImage()` 抛出配置缺失错误。

`InterfaceImageGenerator` 是 Interface 对图像生成的最小需求：

```typescript
interface InterfaceImageGenerator {
...
`listInstances()` 默认返回全部实例，`options.completeOnly` 为 `true` 时只返回文本和图片值均完整的实例。
...
1. 读取 Interface 字段定义。
2. 为全部 `varName` 建立空字符串值，并以 `name`（去除首尾空格，缺省为 `未命名题组`）作为实例名称。
...
`InterfaceInstanceEdit` 描述一次整表更新：

```typescript
interface InterfaceInstanceEdit {
  name: string
...
`save()` 接收当前实例名称、全部变量值、图片提示词中间值和可选的图片变量字节，不提供单字段保存。`imageFiles` 以图片变量名为 key：二进制表示新增或替换，`null` 表示移除，缺少 key 表示保留。
...
`replaceFromJson()` 先解析 JSON，再使用 Interface 字段树生成的 JSON Schema 校验结构。校验由 TypeBox Value 解释执行，不依赖 `eval` 或 `Function` 动态代码生成，兼容 renderer 的严格 CSP。成功后，文本字段映射到 `values`；图片字段的提示词映射到 `imagePrompts`，并按 `options.imageProvider` 指定的图像 Provider 重新生成图片，再把文本、提示词和图片一起原子替换。调用方未指定 `imageProvider` 时由图像适配器决定默认选择。
...
`generateImage(prompt, options?)` 为单个提示词生成一张图片：

- 图片由 `imageGenerator` 生成；未配置图像适配器时抛出配置缺失错误。
- `options.provider` 指定图像 Provider；未指定时由图像适配器决定默认选择。
- `options.signal` 可取消请求，取消时抛出 `AbortError`。
- 生成结果校验为受支持的图片格式后，以 `Uint8Array` 返回，不写入实例资源。

该入口供 renderer 在保存题组前生成单个图片字段的预览；结果进入待保存表单状态，由调用方在 `save()` 时提交。
...
该端口表示 Interface 对文本生成的最小需求，不要求 AIRouter 使用相同命名。renderer 通过 `InterfaceAIRouterAdapter` 把 AIRouter 文本流适配到该端口。
...
interface InterfaceAIGenerationHandle extends TaskProgressHandle<InterfaceAIGenerationResult> {
  retry(): Promise<InterfaceAIGenerationHandle>
}
```

`retry()` 只在 `completion` 返回 `failed` 后允许调用一次，并复用本次会话已完成的推理、校验和图片结果。
...
4. 创建 `InterfaceAIGenerationHandle`。
5. 流结束后校验完整 output JSON。
6. 存在图片提示词时逐张调用图像 Provider 生成图片。
7. 全部成功后覆盖并保存当前实例。
8. 释放实例锁。
...
- Interface 采用按规范化内容计算的内容 ID，而不是预设 UUID：相同内容在任何设备上得到相同 ID，因此用户发布内容与内置内容可以按内容复用和去重，不需要维护预设 ID 表；修改已发布内容后自然得到新 ID，Template 引用不会被静默改写。
- 同一 ID 对应不同规范化内容只可能是哈希碰撞或数据篡改，因此在任何写入前拒绝，而不是覆盖已有内容；否则引用会在无提示的情况下指向另一份内容。
- 草稿使用独立随机 UUID，与内容 ID 解耦，编辑中的不完整内容不占用也不污染发布 ID 空间。
- 实例使用实体身份而不是内容身份：重新生成、复制、另存都产生新 UUID，values 相同也不是同一实例；从交换文件或备份恢复时保留原 UUID，因此重复导入可以识别同一实体。
- 同一 ID 在物理上只能存在于 `published` 或某个 `builtin/<builtinKey>/versions` 之一，避免同一内容在两个分区各有一份可独立演化的副本。
