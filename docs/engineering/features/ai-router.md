...
`@ls101/airouter` 为 Electron main 和 renderer 提供文本生成、图像生成、语音合成与语音识别基础设施。文本、图像、语音合成和语音识别 Provider 使用相互独立的普通配置、模型列表和加密密钥。AIRouter 不保存业务图片或录音文件。
...
语音识别 Provider 区分在线和本地两种运行方式。在线 Provider 类型为 `openai-compatible`，配置 Base URL、API Key 和模型；本地 Provider 类型为 `qwen3-asr`，选择一个已导入的 Qwen3 ASR 模型包并启用其中的模型。Provider 使用独立配置和密钥范围：

```text
配置：config/airouter/speech-recognition-providers.json
密钥：airouter/speech-recognition-providers/{providerId}
```

`AIRouterSpeechRecognitionModelOption` 描述一个可选的 Provider 与模型组合：

```typescript
interface AIRouterSpeechRecognitionModelOption {
  providerId: string
  providerName: string
  modelId: string
  modelName: string
...
本地模型来自用户导入的 ZIP 模型包。模型包 manifest 的 `runtime.engine` 必须为 `qwen3-asr`，并声明 `convFrontend`、`encoder`、`decoder`、`tokenizer` 和 `vad` 资产；导入时校验 manifest、资产大小和 SHA-256，相同哈希的资源只保存一份 Blob。
...
请求必须指向已配置且已启用该模型的 Provider，音频必须非空、不超过 100 MB，且媒体类型以 `audio/` 开头。在线 Provider 把音频以 multipart 提交到 `{baseUrl}/audio/transcriptions` 并读取响应中的 `text`。本地 Provider 交给惰性初始化的持久 Worker；Worker 先用 FFmpeg 转换为 16 kHz 单声道 PCM WAV，再用 Silero VAD 分段，依次调用 sherpa-onnx Qwen3 离线识别并按顺序拼接文本。临时音频目录在每次请求结束后删除，模型本身留在 Worker 中供后续请求复用。

识别使用独立的 start/result/error/abort IPC 通道。renderer client 接受 `AbortSignal`；取消会终止当前本地 Worker、拒绝尚未完成的识别并在下一次请求时重新初始化，以确保原生推理不会在后台继续占用资源。业务调用按顺序发起识别，尚未提供并发调度。

`设置 → AI 引擎 → 语音识别` 已接入独立设置页，用于管理语音识别 Provider 和 ASR 模型包：列出、创建、编辑和删除在线或本地 Provider；在线 Provider 配置 Base URL、独立 API Key 和模型 ID，本地 Provider 选择模型包并启用模型；页面同时提供 ZIP 模型包导入与删除。已保存的 API Key 默认不返回 renderer，留空保存表示继续使用原密钥。AI 评分会话通过 `listSpeechRecognitionModels()` 枚举已配置 Provider 的已启用模型供用户选择。
...
Provider 表示实际的后端或本地运行时。在线 Provider 当前只支持 OpenAI Compatible 格式；离线 Provider 支持 Pocket TTS WASM 和 Qwen3-TTS 0.6B 本地运行时。一次语音调用的具体目标由 Provider、模型和音色共同决定：
...
在线 Provider 配置 Base URL、API Key 和可用模型。离线 Provider 不需要 Base URL 或 API Key，而是选择一个兼容的模型包，然后勾选启用的模型和音色。同一个模型包可以被多个离线 Provider 配置引用，以支持不同的模型或音色组合。`qwen-tts` Provider 另有 `backend` 字段，当前设置页保存为 `cpu`。
...
本地 Provider 可选 `pocket-tts` 或 `qwen-tts`。编辑器按 manifest 的 `runtime.engine` 过滤模型包：

- 没有任何可用模型包时，隐藏模型和音色配置，展示“需要先导入 <运行时> 模型包”的空状态和导入按钮。本地 Provider 可以先保存为空配置，但没有模型包、启用模型和启用音色时不能用于合成。
...
当前语音合成设置页只管理 Provider 和模型包，不保存 `default`、`man`、`woman` 角色路由。三组 Provider、模型和音色选择器由发起语音合成的业务界面提供：`TemplateExamGenerationPage` 的“语音设置”区域为 `default`、`man`、`woman` 分别渲染角色选择器，并把结果作为 `AIRouterSpeechRouting` 传给 `createExamGenerationSession()`。角色路由只在当前生成会话中使用，不写入设置或 Template 工作文档。
...
自动化测试覆盖独立配置和密钥、按 Provider 类型分派、两类文本协议的成功与失败流、调用时 Provider 选择、图片请求校验和二进制返回、IPC 取消、手动请求队列、模态框文件与剪贴板导入、手动测试生成、Interface 图片进度与原子资源保存。语音识别测试覆盖 ASR 模型包导入与资源复用、仅暴露已启用模型、在线 OpenAI-compatible multipart 请求、输入拒绝、main IPC 事件转发、renderer 取消和设置页 Provider 渲染；常规自动化测试不加载 Qwen3 ASR 模型执行真实推理。真实远端 Provider 的付费调用未纳入自动化测试。
...
- `packages/renderer/src/features/airouter/AIRouterSettingsPage.tsx`
- `packages/renderer/src/features/airouter/AIRouterSpeechRecognitionSettingsPage.tsx`
- `packages/renderer/src/features/templates/TemplateExamGenerationPage.tsx`（语音合成角色路由）
