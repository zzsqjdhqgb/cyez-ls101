...
- 当前配置使用明文 JSON，`@ls101/config-store` 自身不提供加密或密钥存储，不适合直接保存需要加密保护的 API Key。应用层的 `@ls101/secret-store` 提供基于 Electron `safeStorage` 的加密密钥存储，AIRouter 的文本、图像、语音合成和语音识别 Provider 已使用它保存 API Key。
- 当前没有多窗口或外部文件修改同步。
