...
`@ls101/interface-editor` 已使用该契约包装 Interface AI 文本生成、结果校验和实例保存流程。题组编辑页已通过 `useSyncExternalStore` 接入该句柄，展示流式日志、步骤状态、取消入口和失败重试；当前仍未实现跨模块通用进度列表组件。
...
`@ls101/interface-editor` 的 AI 生成在通用句柄上增加失败重试：
...
`retry()` 只在 `completion` 返回 `failed` 后允许调用一次，并保留本次生成中已经完成的推理、校验和图片结果，从失败阶段继续；已完成或已取消的生成调用 `retry()` 会抛出错误。
...
- `running` 表示步骤正在执行。
- `completed` 表示步骤已经结束。
- `failed` 表示步骤执行失败；当前状态集合不包含 `cancelled`，取消属于整个业务操作的最终结果，由 `completion` 的结果类型表达。
- 领域实现在失败时把正在运行的任务项置为 `failed`，并在可选日志中记录错误消息；UI 仍应以 `completion` 结果作为最终业务状态来源。
...
`completion` 返回 `failed` 时，`InterfaceAIGenerationHandle.retry()` 会在同一个生成会话上启动新的尝试并返回新的句柄，复用已经完成的推理、校验或图片结果；原句柄的 `completion` 仍保留失败结果。同一失败会话只允许重试一次。
...
- 失败任务项置为 `failed`，并从失败阶段重试而不重复生成已完成的文本和图片。
