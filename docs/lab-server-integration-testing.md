...
原始数据导出与彻底清除：`disaster-recovery.test.ts` 使用损坏数据库／配置和 WAL、日志、旧目录验证原样导出，覆盖磁盘写满、活动服务、目录占用、路径重叠、链接、导出篡改、原数据变化、服务待删除及部分删除重试。`local-recovery.test.ts` 验证保存位置和校验凭据只能由宿主选择及保存，取消或导出失败不能清除。教师端跨端用例覆盖安装失败后仍能进入恢复页、导出失败、输入确认文字、二次确认、取消与失败重试。测试只操作临时目录，系统服务命令使用模拟；实际 Windows／Linux 提权和系统注册清理需要目标系统验证。

故障停止回归：`emergency-stop.test.ts` 模拟两个系统管理器，覆盖失联时绕过服务控制通道、停止超时升级、身份不符、命令失败和残留进程占用；`emergency-stop-windows.test.ts` 仅在 Windows 执行真实 PowerShell 决策脚本，替换系统命令以验证正常停止、强杀、PID 变化、共享进程与失败结果，不操作真实服务。生命周期测试核对恢复标记、启动互斥、维护模式和已提交回执；教师端跨端用例核对确认、取消、失败重试及启动入口。真实 UAC、SCM／systemd 停止和进程树回收仍需目标系统验收，模拟结果不能替代这些证据。
