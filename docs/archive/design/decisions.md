...
- 导入时本地已有相同 ID 且规范化内容一致，则跳过定义写入并继续处理选中的实例；用户无需拥有一份与当前内置内容完全相同的副本
...
- 导入 Interface 时复算内容 ID；本地已有同 ID 且内容一致时跳过定义写入，并继续处理选中的实例
- 不同 `instanceId` 的实例全部保留，不根据实例内容去重
- 同 `instanceId`、同内容时跳过实例；同 `instanceId`、不同内容时在任何写入前拒绝整个导入
