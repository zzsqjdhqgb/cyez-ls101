...
superseded-by: docs/engineering/features/interface-editor.md
-->
...
创建题组时立即生成 UUID v4并持久化一份名称为“未命名题组”、所有变量值为空字符串的正式题组，然后进入题组编辑页面。题组页面采用整表保存，不按单字段写入。用户可以手动编辑题组 `name` 和全部值，也可以通过 JSON 或 AI 生成结果原子覆盖当前题组的全部值；`name` 不进入 JSON Schema，也不会被 JSON 或 AI 修改，覆盖不改变 `instanceId`。JSON 解析或结构校验失败、AI 失败或取消时，原值保持不变。已有值允许再次通过 JSON 或 AI 覆盖，覆盖后仍可继续手动编辑。

AI Router 提供自身规范的文本流，Interface 将其适配为通用任务进度句柄。UI 展示 AI 输出或模型提供的思考日志，并继续展示 Interface 自身的结果校验和实例保存步骤。图片生成使用普通异步接口，Interface 可为每张图片建立无日志的进度项。一个实例同时只允许一个 AI 生成任务；生成期间禁止保存手动编辑结果或执行 JSON 覆盖。用户可在保存开始前取消生成；原子保存开始后任务不可取消，UI 禁用取消操作直至写入结束。

AI 生成中途失败时，当前会话允许从失败步骤重试。文本流失败时重新执行文本生成；图片失败时保留已完成的文本、校验结果和图片，从失败图片继续；保存失败时只重试保存。检查点仅保留在当前内存会话中，全部步骤成功前仍不覆盖实例；取消任务或关闭对话框会丢弃检查点。
