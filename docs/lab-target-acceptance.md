...
自动套件 `yarn vm:lab`（一次性 Windows Server 2022 虚拟机，设计与用例见 [lab-vm-acceptance-design.md](lab-vm-acceptance-design.md)）自 2026-09-17 起在真实 Windows 宿主机上全绿，当时的 15 项 M1 与 11 项 M2 用例均已通过。它**只**提供下表标注的自动证据，其余条目仍需人工执行；测试通过不等于本节通过。

2026-09-19 起，套件新增了升级、卸载与数据保留用例（M4，U1–U5：同版本重装、版本变化的准入拒绝、真实准备后的升级、NSIS 客户端卸载、服务卸载与重装保留数据）。这些用例的**代码与容器内自证已完成，但尚未在真实 Windows 上运行**——下表条目 6、7 的"自动证据"一栏因此仍写"待运行"，只有下一次 `yarn vm:lab` 全绿才能改。
...
| 6    | 待运行   | 已有 U1/U2 覆盖同版本重装的免准备、版本变化的准入拒绝、以及经真实备份准备后的升级与数据保留；尚未在真实 Windows 上执行，也**不**覆盖 Linux deb 的升级路径 |
| 7    | 待运行   | 已有 U3/U4 覆盖 NSIS 卸载保留服务与数据、服务卸载后重装的 serverId 与回执保留；学生端登录自启动、`.lsjoin` 关联与二次启动绑定命令仍属 M3/M4 未实现部分（学生端安装包本轮只做上传校验，未安装）                                   |
