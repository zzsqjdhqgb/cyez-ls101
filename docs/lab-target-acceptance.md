...
自动套件 `yarn vm:lab`（一次性 Windows Server 2022 虚拟机，设计与用例见 [lab-vm-acceptance-design.md](lab-vm-acceptance-design.md)）自 2026-09-17 起在真实 Windows 宿主机上全绿：M1 15 项、M2 11 项，以及自 2026-09-21 起 M4 的 8 项（同版本重装、版本变化的两次准入拒绝、真实准备后的直接安装器升级、NSIS 客户端卸载、服务卸载与重装保留数据、第二遍机密扫描）。它**只**提供下表标注的自动证据，其余条目仍需人工执行；测试通过不等于本节通过。

M4 还确认了两条部署步骤，已写入 [lab-vm-acceptance-design.md](lab-vm-acceptance-design.md) 第 12.2 节：升级完成后服务处于停止状态（需显式启动），且仍处于维护模式（需显式退出）。
...
| 4    | 部分     | 试卷上传下载、作答归档、回执幂等已由 N7 在真实宿主机上跑通；断电时的目录 fsync 仍为人工确认                                  |
| 5    | 部分     | 服务启停（S14）、维护准入与租约边界（N8/N10）已在真实宿主机上通过；注销与重启后的自启动属 S16、G8，尚未实现                  |
| 6    | 是（Windows） | M4 已于 2026-09-21 在真实 Windows 宿主机的 `yarn vm:lab` 中全绿：同版本重装的免准备、版本变化的两次准入拒绝、经真实备份准备后的直接安装器升级、serverId/指纹/试卷/回执保留。**不**覆盖 Linux deb 的升级路径（见"Linux systemd 机器"一节）                 |
| 7    | 部分     | 客户端卸载保留服务与数据、服务卸载后重装均已有自动证据（M4）。学生端登录自启动、`.lsjoin` 关联与二次启动绑定命令仍属 M3 未实现部分；学生端安装包本轮只做上传校验，未安装                                          |
