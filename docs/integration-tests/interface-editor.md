...
每条路径都用独立的临时 Electron `userData` 目录。已发布的题型通过 `fileStore` bridge 直接写入 `FileInterfaceRepository` 的存储布局（`interfaces/published/<interfaceId>/interface.json`），interface id 按仓库相同的 canonicalization + SHA-256 规则推导；题组实例由 UI 打开「新建题组」、填写名称并选择进入方式后创建，不从外部写入。
...
测试路径：`题型列表 -> 题型详情 -> 命名创建题组 -> AI 生成并覆盖 -> 选模型 -> mock /v1/chat/completions -> 题组保存`。
...
测试路径：`命名创建题组 -> AI 生成并覆盖 -> 选文本模型和图像 Provider -> mock 文本流 + mock /v1/images/generations -> 原子保存`。
...
测试路径：`mock-nonjson 非 JSON 输出 -> 校验失败 -> 检查 JSON 对话框 -> mock-slow 生成 -> 取消生成`。

操作流程：先用返回非 JSON 文本的模型触发校验失败，检查原题组内容未被覆盖并在 JSON 对话框看到原始输出与解析错误；再启动慢速生成并在运行中取消。

测试内容：校验失败状态「生成内容未通过校验」、字段错误数量、JSON 对话框的原始输出和解析错误均正确；取消后显示「生成已取消」与「已取消 AI 生成」；两种失败路径都不会写入实例数据。

### IE-04～IE-06 草稿生命周期

覆盖从题型库“草稿”页签创建草稿、基本信息和字段保存后重新打开、空变量名发布校验、稳定题型发布确认、发布成功、草稿复制/删除，以及未保存修改离开时的取消和放弃操作。

### IE-07～IE-09 题组生命周期

覆盖命名创建题组、字段值保存后重新打开、未保存修改离开确认、“高级操作”中的 JSON 合法覆盖、已有内容的覆盖确认、非法 JSON 保留原值，以及通过题组行菜单删除后的文件存储清理。`IE-08b` 额外覆盖含图片字段的 JSON 覆盖：对话框只选择图像 Provider，不选择文本模型；JSON 中的图片提示词经真实生图链路生成资产并原子保存。

### IE-10～IE-11 题型复制与交换

覆盖在“题型定义”页签将已发布题型复制为草稿，以及包含题组实例的 `.lsinterface` 导出、清空、从题型库操作菜单重新导入和题组恢复。导出文件同时断言 ZIP 文件头，避免只验证 UI toast。
...
IE-13 通过真实文件选择器、Electron 剪贴板和图片字段 AI 生图验证图片暂存、预览、移除和保存；IE-15 验证仅在存在图片提示词时出现的批量生图任务对话框及其生成、保存、返回和取消。
