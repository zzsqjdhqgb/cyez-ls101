...
覆盖命名创建题组、字段值保存后重新打开、未保存修改离开确认、“高级操作”中的 JSON 合法覆盖、已有内容的覆盖确认、非法 JSON 保留原值，以及通过题组行菜单删除后的文件存储清理。`IE-08b` 额外覆盖含图片字段的 JSON 覆盖：面板只选择图像 Provider，不选择文本模型；JSON 中的图片提示词经真实生图链路生成资产并原子保存。
...
IE-13 通过真实文件选择器、Electron 剪贴板和图片字段 AI 生图验证图片暂存、预览、移除和保存；IE-15 验证仅在存在图片提示词时出现的批量生图面板及其生成、保存、完成和取消。
