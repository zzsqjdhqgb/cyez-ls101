...
它拥有：外观偏好、软件数据位置、激活状态、AI 服务商 / 模型 / 密钥配置、本地模型包与扩展包状态。
...
- AI 引擎：文本生成、图像生成、语音合成、语音识别与 AI 语音评测五个区域，各自管理服务商、模型、密钥、连接测试与可用状态。
...
**边界约束**：只有真正影响当前任务的配置才在题组生成、试卷生成或 AI 评分的设置步骤中出现；服务商管理本身不产生题组、试卷或评分结果。

## 状态与生命周期

- **外观偏好**：修改后立即作用于当前窗口并持久化；保存失败回滚到上一次的值；可恢复默认设置。
- **服务商**：新建草稿、填写协议与地址、测试连接、保存、发现与管理模型、使用。测试失败或模型发现失败保留草稿且不写入配置。
- **已保存服务商**：可编辑配置、启用或禁用模型、删除。删除服务商会同时删除其加密密钥。
...
- **不可逆操作**：删除旧数据目录与删除服务商都需要二次确认；删除服务商不影响已经保存的题组、试卷和作答记录。

## 术语

设置、外观、AI 引擎、服务商、模型、API 密钥、数据目录、旧数据目录、扩展包。详见 [`../glossary.md`](../glossary.md)。
