...
M2 的首次全绿运行：2026-09-18，runId `1789700556225-8611e188-d83c-41fc-9907-4e3fe01aea6b`，宿主机侧 19 步、guest 阶段 26 步（M1 15 + M2 11）全部 `passed`，宿主机对打（N13）与防火墙门控（加规则前不可达、加规则后可达）同时通过。几处值得留档的真机读数：错指纹时 `connections:1 / requests:0 / requestBytes:0`；本机 LAN 地址上的三种认证尝试全部 `401 AUTH_REQUIRED`（含伪造 `X-Forwarded-For`）；入网六种负例各自的状态码；心跳停 22 秒后离线但 `number`/`phase` 保留；32 连接 × 120 秒共 1280 次心跳、0 传输错误、`TimeWait` 21 且动态端口范围 49152+16384 未被消耗；试卷 4.2 MB、作答归档 3.1 MB 的独立摘要一致；租约到期前后 `maintenance-exit` 由 `409 RESOURCE_BUSY`（blocker 指名 `active-task-lease`）转为 `200 normal`；并发上限 `1×201 + 11×429` 与 `64 + 16×503`。
