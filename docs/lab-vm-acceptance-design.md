...
| 服务注册     | WinSW XML `resources/lab/windows/LS101Lab.xml`：`startmode=Manual`、`stoptimeout 1900 sec`、`onfailure restart`、日志 roll-by-size                                                                               |
...
| S15 | 设备处于 `practicing`/`testing` 时停止被 `RESOURCE_BUSY` 阻塞；维护模式下可正常停止                                        | `stoptimeout 1900 sec` 与 WinSW `stopexecutable` 只在真 SCM 下有效                                                   |
