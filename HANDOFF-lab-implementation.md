...
**That hang was the open risk 2, and it is now fixed in the product**: `teacher.nsh` skips the `MessageBox` in silent installs, prints the exit code and the captured installer output, sets exit level 2 and still aborts.

**M4 is now green on a real Windows host.** Run `1789991140621-bca543c3-bb5f-4472-b4e0-8e3355e7215c` (2026-09-21, guest phase 11:52:49 → 11:59:58) passed all **34** guest steps — M1 15, M2 11, M4 8 — with the host peer check and the firewall gate, and destroyed the VM itself. Teacher artifact `84bcbbdc…688ad`, student `546ec570…c6e1`.

Defects this milestone found and fixed, all of which only a real machine shows: the silent-install dialog hanging unattended installs; the install script racing the SCM (`Stop the service before installation or upgrade` while the service was already stopped); a silent failure leaving no readable reason; and an uninstaller whose return is not the end of the uninstall (`_?=` must be quoted, and the files must be polled for). Two deployment steps came out of it and are documented in design §12.2: after an upgrade the service is **stopped** and still in **maintenance**, so deployment has to start it and leave maintenance.

Runner defects fixed along the way are worth knowing about, because each cost a VM cycle: the secret scan demanding secrets that did not exist yet; an installer run against a live service; `reinstallTargets` comparing the package against itself; the version-change precondition being undone by the previous case's cleanup; retention checks asking about a submission that was never uploaded; the probe harness answering `Unknown probe: -Probe` because `&` does not parse a command line; and the run watcher adopting a finished run.
